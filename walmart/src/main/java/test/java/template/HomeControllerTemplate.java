...
public class HomeControllerTemplate implements TemplateLoader {
    ...
    public void load() {
        ...
        Fixture.of(Partners.class).addTemplate(PARTNERS_CREATE_REQUEST, new Rule() {{
            //add("id", "1");
            add("partnersName", "Facebook");
            add("productName", "Product facebook test");
        }});

        Fixture.of(Partners.class).addTemplate(PARTNERS_CREATE_CONFLICT_REQUEST, new Rule() {{
            add("id", "1");
            add("partnersName", "Google");
            add("productName", "Product test");
        }});

        Fixture.of(Partners.class).addTemplate(PARTNERS_LIST_REQUEST, new Rule() {{
            add("partnersName", "Facebook");
            add("productName", "Product test");
        }});

        Fixture.of(Partners.class).addTemplate(PARTNERS_UPDATE_REQUEST, new Rule() {{
